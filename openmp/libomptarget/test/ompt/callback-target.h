        ...
        const void *codeptr_ra)
{
  ...
  {
    case ompt_scope_begin:
      printf("%" PRIu64 ":" _TOOL_PREFIX " ompt_event_target_begin: task_id=%" PRIu64
      ", target_id=%" PRIu64 ", device_num=%" PRIu32 ", kind=%s" ", codeptr_ra=%p" "\n",
          ompt_get_thread_data()->value, task_data->value, target_id, device_num,
          ompt_target_t_values[kind], codeptr_ra);
      break;
    case ompt_scope_end:
      printf("%" PRIu64 ":" _TOOL_PREFIX " ompt_event_target_end: task_id=%" PRIu64
      ", target_id=%" PRIu64 ", device_num=%" PRIu32 ", kind=%s" ", codeptr_ra=%p" "\n",
          ompt_get_thread_data()->value, task_data->value, target_id, device_num,
          ompt_target_t_values[kind], codeptr_ra);
      break;
...
static void on_ompt_callback_target_data_op(
        ...
        const void *codeptr_ra)
{
  printf("%" PRIu64 ":" _TOOL_PREFIX " ompt_event_target_data_op: target_id=%" PRIu64 ", host_op_id=%" PRIu64
  ", optype=%s, src_addr=%p, src_device_num=%" PRIu32 ", dest_addr=%p, dest_device_num=%" PRIu32
  ", bytes=%" PRIu64 ", codeptr_ra=%p" "\n", ompt_get_thread_data()->value, target_id, host_op_id,
          ompt_target_data_op_t_values[optype], src_addr, src_device_num, dest_addr, dest_device_num, bytes, codeptr_ra);
}

static void on_ompt_callback_target_map(
        ompt_id_t target_id,
        unsigned int nitems,
        void **host_addr,
        void **device_addr,
...
{
  char buffer[2048];
  printf("%" PRIu64 ":" _TOOL_PREFIX " ompt_event_target_map: target_id=%" PRIu64
  ", nitems=%" PRIu32 ", codeptr_ra=%p" "\n", ompt_get_thread_data()->value, target_id,
          nitems, codeptr_ra);
  for (int i = 0; i < nitems; i++)
  {
    format_mapping_flag(mapping_flags[i], buffer);
...
static void on_ompt_callback_device_mem(
        ompt_data_t *target_task_data,
        ompt_data_t *target_data,
