...
Node* IntervalTree::rightRotation(Node* head){
    ...
    newhead->height = 1+max(height(newhead->left_child), height(newhead->right_child));
    return newhead;
}
...
Node* IntervalTree::insertUtil(Node* head, const Interval &interval, const MapInfo &info){
    Node *n = nullptr;
    ...
    head->height = height(head->left_child) > height(head->right_child) ? 1 + height(head->left_child) : 1 + height(head->right_child);
...
Node* IntervalTree::removeUtil(Node* head, const Interval &i){
    ...
    if(bal>1){
        if(height(head->left_child) >= height(head->right_child)){
            return rightRotation(head);
        }else{
            head->left_child = leftRotation(head->left_child);
            return rightRotation(head);
        ...
        if(height(head->right_child) >= height(head->left_child)){
            return leftRotation(head);
        }else{
            head->right_child = rightRotation(head->right_child);
            return leftRotation(head);
